...
**Write snapshot after every major step.** As you go, track step-level progress in `meta.progress.map`: set `current_step` when a step begins, append it to `completed_steps` when it finishes. On a fresh invocation, if `meta.progress.map` exists with `current_step` ≠ `"done"`, resume from the first step not in `completed_steps` instead of restarting.
...
Then write the snapshot one final time with `map` added to `meta.skills_run` and `meta.progress.map.current_step` set to `"done"`.
