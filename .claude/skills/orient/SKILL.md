...
- **If `.archeology/report.md` does not exist**, create it with this header:
  ...
  ```
- **Write your section** between markers, just after the header. If the block already exists, replace its contents; otherwise insert it. Keep sections in this order: `orient`, `map`, `quality`, `the-finder-outer`, `story`.
  ```markdown
  <!-- section:orient -->
