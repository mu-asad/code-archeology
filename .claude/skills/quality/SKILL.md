...
- Insert or replace your marker-delimited section. Keep section order `orient`, `map`, `quality`, `story`:
  ```markdown
  <!-- section:quality -->
  <the same content you printed to the console, verbatim (including the craft narrative in full) — it already begins with `## Quality Assessment: [grade]`, so do not add another heading>
  <!-- /section:quality -->
  ```
